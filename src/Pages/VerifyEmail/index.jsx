...
import { CheckCircle, XCircle, Loader2 } from 'lucide-react'; // Assuming you're using lucide-react for icons
...
const VerifyEmail = () => {
  ...
  useEffect(() => {
    Global.httpPut(`/auth/verify/${token}`)
      .then(res => {
        setStatus('success');
        setTimeout(() => {
          Global.user.isVerified = true;
          navigate(Global.user ? "/profile" : "/login");
        }, 2000);
      })
      .catch(err => {
        console.error(err);
